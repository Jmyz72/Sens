...
- DB lives in the Tauri app-data dir; every connection sets `PRAGMA foreign_keys = ON`. Migrations are append-only in `db/migrations.rs`, each in its own transaction; first-run seeding + the `seeded` flag commit atomically. **Default categories seed once** (idempotent insert-or-ignore; the seed inserts only top-level categories, deduped by the partial unique index `idx_categories_top_kind_name` on `(kind, name) WHERE parent_id IS NULL`); **system categories cannot be archived**.
